...
int main() {
	...
	char filename[256];
   	 ...
   	 // Classes 4 and 5 are "Elephants" -> CPU Bound (The Noise)
   	 if (entry.func_class >= 4) {
       		 strcpy(filename, "./fib_alps");
    	} 
    	// Classes 1, 2, 3 are "Mice" -> I/O Bound (The Victim)
    	else {
        	strcpy(filename, "./io_task");
    	}
   	 // ---------------------------------

    	printf("Executing workload entry %d (%s) Class:%d\n", entry.id, filename, entry.func_class);
